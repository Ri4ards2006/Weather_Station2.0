...
void loop() {
  ...
  if (millis() - lastUpdate >= updateInterval) {
    ...
    currentTime = String(now.hour()) + ":" + (now.minute()<10?"0":"") + String(now.minute());
...
void updateWeatherUI(float temp, float humidity, float pressure, float distance, int mqValue) {
  ...
  Serial.print(F("🌡  Temp: ")); Serial.print(temp); Serial.println(" C");
  Serial.print(F("🌬 Druck: ")); Serial.print(pressure); Serial.println(" hPa");
  Serial.print(F("📏 Distanz: ")); Serial.print(distance); Serial.println(" cm");
  Serial.print(F("🌫️ Luftqualität (MQ135): ")); Serial.println(mqValue);
  Serial.print(F("⏰ Aktualisiert: ")); Serial.println(currentTime);
  Serial.println(F("-------------------------"));
}


// --- Alte Platzhalter-Funktion wird nicht mehr benötigt ---
